#!/usr/bin/env python3
"""
Convert IOS-XE collected training JSONL to clawlab ssh-ops run_command tool_calls format.

Input rows often use role=tool with embedded {"command","device","output"} without a
preceding assistant tool_call. This script inserts run_command tool_calls and maps
device IP/hostname to ssh-ops host names when possible.

Usage:
  python clawlab-ssh-ops/convert_iosxe_to_ssh_ops.py
  python clawlab-ssh-ops/convert_iosxe_to_ssh_ops.py --input 'field_definitions/ios_xe_*.jsonl'
"""
...
def convert_messages(messages: list[dict]) -> list[dict] | None:
    ...
    out: list[dict] = [{"role": "system", "content": CLAWLAB_SYSTEM}]
    i = 0
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "--output",
        type=Path,
        default=SCRIPT_DIR / "iosxe_ssh_ops_converted.jsonl",
    )
